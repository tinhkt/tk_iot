...
class DeviceProvider with ChangeNotifier {
  ...
  // Nhận diện tên TỰ SINH của Backend (khớp CHÍNH XÁC công thức trong hassEndpointsFromState):
  //   sw-{4hex} | sw-{4hex}-{n} | Fan-{4hex} | sw-tog-{4hex}
  // last4 là 4 ký tự hex cuối MAC nên tên custom kiểu "sw-bếp" KHÔNG khớp -> không bị nhầm.
  static final RegExp _autoNameRegex =
      RegExp(r'^(sw-tog-[0-9a-f]{4}|(sw|fan)-[0-9a-f]{4}(-\d+)?)$', caseSensitive: false);
  static bool _isAutoGeneratedName(String? name) {
    final n = name?.trim() ?? '';
    return n.isEmpty || _autoNameRegex.hasMatch(n);
  }
  ...
  void updateDeviceStateFromMQTT(String mac, String topic, String payload) {
    ...
    if (json != null) {
      ...
      // [FIX MẤT TÊN KHI TOGGLE] CHỐT CHẶN: sóng state realtime TUYỆT ĐỐI không được ghi đè
      // tên CUSTOM (do user đặt) bằng tên TỰ SINH (sw-xxxx/Fan-xxxx). Nếu update mang tên tự
      // sinh cho một endpoint đang giữ tên custom -> BỎ khóa _name đó, giữ nguyên tên hiện có.
      // (Chỉ áp cho MQTT realtime; hydrate REST không lọc nên đổi/xóa tên vẫn hiệu lực.)
      updates.removeWhere((k, v) =>
          k.endsWith('_name') &&
          _isAutoGeneratedName(v?.toString()) &&
          device.dps.containsKey(k) &&
          !_isAutoGeneratedName(device.dps[k]?.toString()));
